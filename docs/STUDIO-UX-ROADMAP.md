...
| 7 | `visual/colour-atelier` + `visual/modality-polish` | #TBD | §6 modality surfaces: colour picker → fabric atelier (named woven-texture chips via `I18N.colorName`, the dead preview rectangle is gone — the live flat IS the preview; duo hint line, order badges kept, bot-compatible class names); slider → hairline track with live gradient fill (`--val`) + ring thumb + approached-pole emphasis (`data-side`, native input kept for a11y); category cards → six bespoke `GarmentSVG` mini-flats (jacket as cropped boxy zip block vs slim long button shirt — the glyph era's t-shirt/hemd/jacke lookalikes are gone) |
...
Slices 8–10 per §10: `engine/regions-hotspot-board` (§7), `visual/refine-crescendo`
(§8), `engine/sewing-handoff` + `visual/ownership-seam` (§5.4/§9).
